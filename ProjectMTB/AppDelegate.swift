...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        UINavigationBar.appearance().titleTextAttributes = [NSAttributedStringKey.font: UIFont(name: "HelveticaNeue-Medium", size: 16)!,NSAttributedStringKey.foregroundColor: UIColor.white]
        UINavigationBar.appearance().setBackgroundImage(UIImage(), for: UIBarMetrics.default)
        UINavigationBar.appearance().shadowImage = UIImage()
        UINavigationBar.appearance().backgroundColor = UIColor(hex: "#3AD29F")
        UIBarButtonItem.appearance().tintColor = .white

        // Override point for customization after application launch.
        ImageCache.default.maxDiskCacheSize = 5 * 1024 * 1024
        ImageCache.default.maxMemoryCost = 50 * 1024 * 1024
        IQKeyboardManager.shared.enable = true
        (IQKeyboardManager.shared).enableAutoToolbar = false
        if let userdefault=UserDefaults.standard.string(forKey: "user") {
            let user=userLogin(JSONString: userdefault)
            sk.user=user
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        */
        let container = NSPersistentContainer(name: "ProjectMTB")
